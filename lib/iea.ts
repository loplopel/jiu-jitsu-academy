...
export function calculateIEA(i:IEAInput){
  const score=clamp(i.frequency)*.35+clamp(i.attendance)*.25+clamp(i.streak)*.15+clamp(i.trainingMonths)*.10+clamp(i.graduationProgress)*.15;
