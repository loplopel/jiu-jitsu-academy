...
export async function GET(){
  ...
  const [belts,categories,professors]=await Promise.all([
  ...
  const err=belts.error||categories.error||professors.error;
  if(err)return NextResponse.json({error:err.message},{status:500});
  return NextResponse.json({belts:belts.data||[],categories:categories.data||[],professors:professors.data||[]});
}
