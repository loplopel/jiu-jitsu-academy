...
import {Bell,CheckCheck,Clock3,RefreshCw,Send,Settings2,CalendarClock,Cake} from 'lucide-react';
...
const defaults:Settings={class_reminders:true,class_reminder_minutes:120,event_reminders:false,event_reminder_hours:24,birthday_messages:true,achievement_messages:false};
const kindLabels:Record<string,string>={general:'Aviso',class:'Aula',birthday:'Aniversário'};
const kindIcons:Record<string,any>={general:Bell,class:CalendarClock,birthday:Cake};
...
export default function Page(){
  ...
  useEffect(()=>{if(role!=='admin')return;void(async()=>{const[u,s]=await Promise.all([fetch('/api/notifications/users'),fetch('/api/notifications/settings')]);if(u.ok)setUsers(await u.json());if(s.ok){const saved=await s.json();setSettings({...defaults,...saved,event_reminders:false,achievement_messages:false})}})()},[role]);
  ...
  async function runAutomation(){setAutomationStatus('Verificando aulas e aniversários...');const r=await fetch('/api/notifications/automation',{method:'POST'});const j=await r.json();setAutomationStatus(r.ok?`${j.created} nova(s) notificação(ões) automática(s) criada(s).`:j.error||'Falha ao processar.');if(r.ok)await load()}
  ...
  return <AppShell>
    <section className="hero"><div><h1>Comunicação e notificações</h1><div className="muted">Avisos internos da equipe, lembretes de aula e aniversários.</div></div><div className="toolbar"><span className="pill">{unread} não lida(s)</span>{unread>0&&<button className="btn btn-secondary" onClick={()=>mark()}><CheckCheck size={17}/>Marcar todas como lidas</button>}<button className="btn btn-secondary" onClick={()=>load()}><RefreshCw size={17}/>Atualizar</button></div></section>
    ...
    {role==='admin'&&<div className="grid grid-2" style={{marginBottom:18}}>
      <form className="card" style={{padding:20}} onSubmit={send}>
        ...
        <div className="form-stack">
          ...
          <div className="grid grid-2">
            <select className="input" value={kind} onChange={e=>setKind(e.target.value)}><option value="general">Aviso geral</option><option value="class">Aula</option><option value="birthday">Aniversário</option></select>
      ...
      <div className="card" style={{padding:20}}>
        ...
        <div className="form-stack">
          ...
          <small className="muted">O botão “Processar agora” gera somente avisos que ainda não foram criados. A mesma aula ou aniversário não é duplicado.</small>
        </div>
      </div>
    </div>}
    ...
    <div className="card" style={{padding:20}}>
      ...
      <div className="notification-list">{items.map(n=>{const Icon=kindIcons[n.kind]||Bell;return <article key={n.id} className={`notification-item ${n.read_at?'':'unread'}`}>
        <div className="notification-icon"><Icon size={19}/></div><div className="notification-body"><div className="notification-meta"><span className="pill">{kindLabels[n.kind]||'Aviso'}</span><span className="muted">{new Date(n.created_at).toLocaleString('pt-BR')}</span></div><h3>{n.title}</h3><p>{n.message}</p><div className="toolbar">{n.link_url&&!['/ranking','/eventos'].includes(n.link_url)&&<Link className="btn btn-secondary" href={n.link_url} onClick={()=>!n.read_at&&mark(n.id)}>Abrir</Link>}{!n.read_at&&<button className="btn btn-secondary" onClick={()=>mark(n.id)}><CheckCheck size={16}/>Marcar como lida</button>}</div></div>
      </article>})}</div>
    </div>
  </AppShell>
}
