...
export default function Page(){
  ...
  useEffect(()=>{void(async()=>{const sb=getSupabaseBrowserClient();if(!sb)return;const now=new Date();const monthStart=new Date(now.getFullYear(),now.getMonth(),1).toISOString();const six=new Date(now.getFullYear(),now.getMonth()-5,1);const startDay=new Date(now);startDay.setHours(0,0,0,0);const endDay=new Date(now);endDay.setHours(23,59,59,999);
    ...
    const ss=students||[],aa=att||[],cc=classes||[],rr=res||[];const latest=new Map<string,number>();for(const x of iea||[])if(!latest.has(x.student_id))latest.set(x.student_id,Number(x.score));const scores=[...latest.values()];
    const labels:Array<ChartPoint>=[];for(let i=5;i>=0;i--){const d=new Date(now.getFullYear(),now.getMonth()-i,1);const key=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`;labels.push({label:d.toLocaleDateString('pt-BR',{month:'short'}).replace('.',''),value:aa.filter(a=>String(a.checked_in_at).startsWith(key)).length})}
    const byHour=new Map<string,{capacity:number;reserved:number}>();for(const c of cc){const h=new Date(c.starts_at).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'});const item=byHour.get(h)||{capacity:0,reserved:0};item.capacity+=Number(c.capacity||0);item.reserved+=rr.filter(r=>r.class_id===c.id).length;byHour.set(h,item)}
